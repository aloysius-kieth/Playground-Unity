...
namespace com.rfilkov.kinect
{
    ...
    public class KinectManager : MonoBehaviour
    {
        ...
        /// </summary>
        /// <returns>The joint position for color overlay.</returns>
        /// <param name="jointPosKinect">Joint Kinect position</param>
        /// <param name="camera">Camera used to visualize the 3d overlay position</param>
        /// <param name="imageRect">Color image rectangle on the screen</param>
        public Vector3 GetJointPosColorOverlay(Vector3 jointPosKinect, int sensorIndex, Camera camera, Rect imageRect)
        {
            if (jointPosKinect != Vector3.zero)
            {
                // 3d position to depth
                Vector2 posDepth = MapSpacePointToDepthCoords(sensorIndex, jointPosKinect);
                ...
                if (posDepth != Vector2.zero && depthValue > 0)
                {
                    ...
                    if (posColor.x != 0f && !float.IsInfinity(posColor.x))
                    {
                        ...
                        float zDistance = jointPosKinect.z;
