...
namespace TRINAX.Arduino {
    public class TrinaxArduino : MonoBehaviour {
        ...
        public SerialPort GetStream1() { return stream1; }

        public SerialPort GetStream2() { return stream2; }
        ...
        void OnReadSuccess(string line) {
            ...
            line = line.Trim();
            if (AppManager.Instance.IsReady) {
                if (line.Length > 0) {
                    //Debug.Log(line);
        ...
        public void OpenAllArduinoButtons() {
            Debug.Log("Opening all Arduino");
            WriteToArduino(GetStream1(), OPEN_ALL_ARDUINO_BUTTONS.ToCharArray());
            Instance.WriteToArduino(GetStream2(), OPEN_ALL_ARDUINO_BUTTONS.ToCharArray());
        }

        public void CloseAllArduinoButtons() {
            Debug.Log("Closing all Arduino");
            WriteToArduino(GetStream1(), CLOSE_ALL_ARDUINO_BUTTONS.ToCharArray());
            WriteToArduino(GetStream2(), CLOSE_ALL_ARDUINO_BUTTONS.ToCharArray());
        }
